...
server <- function(input, output) {
  ...
  output$table <- renderText({
    paste("Daily yields computed averaging by", input$num2, "the differences between present and", input$num2, "days ago values")
  })
  ...
  dataInput2 <- reactive({
    ...
    clusterExport(cl, c("symbols","checkgroup","range"))
